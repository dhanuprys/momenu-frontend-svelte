	...
	<Card.Root class="max-w-md w-full border-dashed shadow-xs">
		<Card.Content class="pt-8 pb-8 flex flex-col items-center text-center">
			...
			<p class="text-sm text-muted-foreground mb-6 leading-relaxed max-w-[280px]">
				Halaman ini belum dipublikasikan oleh pemilik acara. Data analitik tidak dapat ditampilkan saat ini.
			</p>
...
<div class="min-h-screen bg-muted/30">
	...
	<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
		<!-- ── Project Details ────────────────────────────────────── -->
		<div class="flex flex-col sm:flex-row justify-between gap-3 sm:gap-6 bg-card rounded-xl border p-4 shadow-xs">
			<div class="min-w-0 flex-1 space-y-1.5">
				<div class="flex items-start justify-between gap-2">
					<h1 class="text-lg sm:text-2xl font-bold tracking-tight leading-tight truncate">{project.title}</h1>
				...
				<div class="flex items-center flex-wrap gap-2 text-sm text-muted-foreground">
					<Badge variant="secondary" class="capitalize text-[10px] py-0 h-5 hidden sm:inline-flex">
						{project.event_type.replace('_', ' ')}
					</Badge>
					<span class="hidden sm:inline text-muted-foreground/50">•</span>
					<span class="truncate text-xs sm:text-sm bg-muted/50 px-2 py-0.5 rounded-md font-medium">momenu.id/{project.slug}</span>
				</div>
			</div>

			<div class="flex items-center justify-between sm:flex-col sm:items-end sm:justify-center gap-1.5 pt-3 sm:pt-0 border-t sm:border-t-0 border-border/50 shrink-0">
				<span class="text-[11px] sm:text-xs text-muted-foreground uppercase tracking-wider font-medium">Akses Tautan</span>
		...
		<div class="grid gap-4 grid-cols-2 lg:grid-cols-4">
			...
			<Card.Root class="bg-linear-to-br from-primary/10 to-primary/5 border-primary/20">
				...
				<Card.Content>
					...
					<p class="text-[10px] sm:text-xs text-muted-foreground mt-1 leading-tight">Total kunjungan halaman</p>
				</Card.Content>
			</Card.Root>

			<!-- Unique Guests -->
			<Card.Root class="bg-linear-to-br from-blue-500/10 to-blue-500/5 border-blue-500/20 relative overflow-hidden">
				<Card.Header class="flex flex-row items-center justify-between space-y-0 pb-2">
				...
				<Card.Content>
					...
					<p class="text-[10px] sm:text-xs text-muted-foreground mt-1 leading-tight">Berdasarkan perangkat / IP</p>
				</Card.Content>
				{#if analytics.total_visits > 0}
					<div
						class="absolute bottom-0 left-0 h-1 bg-blue-500/50 transition-all duration-700"
						style="width: {Math.min((analytics.unique_guests / analytics.total_visits) * 100, 100)}%"
					></div>
				{/if}
			</Card.Root>

			<!-- Total RSVP -->
			<Card.Root class="bg-linear-to-br from-emerald-500/10 to-emerald-500/5 border-emerald-500/20 relative overflow-hidden">
				<Card.Header class="flex flex-row items-center justify-between space-y-0 pb-2">
				...
				<Card.Content>
					<div class="text-2xl sm:text-3xl font-bold">{analytics.rsvp_stats.attending + analytics.rsvp_stats.not_attending}</div>
					<p class="text-[10px] sm:text-xs text-muted-foreground mt-1 leading-tight">Tamu sudah merespon</p>
				</Card.Content>
				{#if analytics.unique_guests > 0}
					<div
						class="absolute bottom-0 left-0 h-1 bg-emerald-500/50 transition-all duration-700"
						style="width: {((analytics.rsvp_stats.attending + analytics.rsvp_stats.not_attending) / analytics.unique_guests) * 100}%"
					></div>
				{/if}
			</Card.Root>
			...
			<Card.Root class="bg-linear-to-br from-amber-500/10 to-amber-500/5 border-amber-500/20">
				...
				<Card.Content>
					...
					<p class="text-[10px] sm:text-xs text-muted-foreground mt-1 leading-tight">Total pax yang hadir</p>
				</Card.Content>
			</Card.Root>
		</div>
			...
			<Card.Content class="p-0">
				<div class="flex flex-col sm:flex-row divide-y sm:divide-y-0 sm:divide-x">
					<div class="flex-1 p-5 flex items-center gap-4">
						<div class="h-12 w-12 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center shrink-0">
					...
					<div class="flex-1 p-5 flex items-center gap-4">
						<div class="h-12 w-12 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center shrink-0">
					...
					<div class="flex-1 p-5 flex items-center gap-4">
						<div class="h-12 w-12 rounded-full bg-yellow-100 dark:bg-yellow-900/30 flex items-center justify-center shrink-0">
						...
						<div class="flex items-end justify-between h-48 gap-2 w-full">
							{#each analytics.daily_visits as day}
								<div class="flex flex-col items-center gap-2 flex-1 group h-full">
									<span class="text-xs font-medium text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity">{day.count}</span>
									<div
					...
					<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
						...
						<div class="w-full sm:w-auto overflow-x-auto pb-1 -mb-1">
							<Tabs.List class="flex w-fit min-w-full sm:min-w-0">
								<Tabs.Trigger class="flex-1 sm:flex-none" value="rsvp">Daftar RSVP ({analytics.total_rsvps})</Tabs.Trigger>
								<Tabs.Trigger class="flex-1 sm:flex-none" value="visits">Kunjungan Terakhir</Tabs.Trigger>
							</Tabs.List>
						</div>
					</div>
				</Card.Header>
											...
											<Table.Row>
												...
												<Table.Cell>
													...
													{:else if rsvp.attending}
														<Badge class="bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400 gap-1">
												...
												<Table.Cell class="max-w-[200px]">
													{#if rsvp.special_message}
														<p class="text-sm text-muted-foreground truncate" title={rsvp.special_message}>
