	...
	<div class="container mx-auto max-w-6xl px-4 md:px-8">
		...
		{#if loading}
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
				{#each Array(3) as _}
					<div
						class="flex flex-col group rounded-[2rem] border border-stone-200/80 bg-white/90 backdrop-blur-xl overflow-hidden shadow-xs p-3"
					>
		...
		{:else}
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
				{#each filteredThemes as theme}
					{@const style = themeCardStyles[theme.id] || {
						bg: 'bg-[#121815] border-stone-800 text-stone-100',
						text: theme.name,
						badge: 'Premium'
					}}
					<div
					...
					>
						<!-- Card Header Preview -->
						<div
							class="h-64 relative overflow-hidden flex items-center justify-center p-6 border-b border-stone-200/40 {style.bg}"
						>
							<!-- Inner Mock Screen -->
							<div
							...
							>
								...
								>
									<span
										...
										>PREVIEW TEMA</span
									>
									<span class="font-serif italic text-base font-normal text-emerald-100 truncate"
										>{theme.name}</span
									>
									<div class="h-0.5 w-6 bg-emerald-400/40 mx-auto my-1"></div>
									<span class="text-[8px] uppercase tracking-wider text-stone-400"
										>{categoryLabels[theme.event_type] || theme.event_type}</span
									>
								</div>
								<div
									class="text-[7px] text-center text-emerald-200/70 border-t border-white/10 pt-2 font-semibold uppercase tracking-widest"
								>
									Buka Undangan
								</div>
							</div>
						...
						<div class="p-7 flex-grow flex flex-col justify-between space-y-6">
							<div class="space-y-2">
								<div class="flex items-center justify-between">
									<h3
										class="font-serif font-normal text-xl text-stone-900 group-hover:text-emerald-900 transition-colors"
									>
										{theme.name}
									</h3>
									<span
										class="text-[10px] uppercase tracking-wider font-semibold text-stone-500 bg-stone-100 px-3 py-1 rounded-full border border-stone-200"
									>
								...
								<p class="text-xs text-stone-500 leading-relaxed font-sans line-clamp-2">
									{theme.description ||
										'Desain elegan berestetika tinggi dengan navigasi mulus, konfirmasi RSVP, dan buku tamu online.'}
								</p>
							</div>
